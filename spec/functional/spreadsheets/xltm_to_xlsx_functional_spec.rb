...
palladium = PalladiumHelper.new(X2t.new.version, 'Xltm to Xlsx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('xltm/')
file_data = nil
describe 'Conversion xltm files to xlsx' do
  (files - result_sets.map { |result_set| "xltm/#{result_set}" }).each do |file|
