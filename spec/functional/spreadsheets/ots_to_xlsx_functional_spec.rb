...
palladium = PalladiumHelper.new(X2t.new.version, 'Ots to Xlsx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('ots/')
file_data = nil
describe 'Conversion ots files to xlsx' do
  (files - result_sets.map { |result_set| "ots/#{result_set}" }).each do |file|
