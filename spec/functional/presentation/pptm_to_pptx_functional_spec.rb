...
palladium = PalladiumHelper.new(X2t.new.version, 'Pptm to Pptx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('pptm/')
file_data = nil
describe 'Conversion pptm files to pptx' do
  (files - result_sets.map { |result_set| "pptm/#{result_set}" }).each do |file|
