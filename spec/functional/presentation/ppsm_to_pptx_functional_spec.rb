...
palladium = PalladiumHelper.new(X2t.new.version, 'Ppsm to Pptx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('ppsm/')
file_data = nil
describe 'Conversion ppsm files to pptx' do
  (files - result_sets.map { |result_set| "ppsm/#{result_set}" }).each do |file|
