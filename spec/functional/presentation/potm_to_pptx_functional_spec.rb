...
palladium = PalladiumHelper.new(X2t.new.version, 'Potm to Pptx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('potm/')
file_data = nil
describe 'Conversion potm files to pptx' do
  (files - result_sets.map { |result_set| "potm/#{result_set}" }).each do |file|
