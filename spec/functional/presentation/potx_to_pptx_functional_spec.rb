...
palladium = PalladiumHelper.new(X2t.new.version, 'Potx to Pptx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('potx/')
file_data = nil
describe 'Conversion potx files to pptx' do
  (files - result_sets.map { |result_set| "potx/#{result_set}" }).each do |file|
