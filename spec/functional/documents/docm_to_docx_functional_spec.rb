...
palladium = PalladiumHelper.new(X2t.new.version, 'Docm to Docx')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.get_files_by_prefix('docm/')
file_data = nil
describe 'Conversion docm files to docx' do
  (files - result_sets.map { |result_set| "docm/#{result_set}" }).each do |file|
